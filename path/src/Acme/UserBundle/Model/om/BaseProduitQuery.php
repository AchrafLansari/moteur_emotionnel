...
use Acme\UserBundle\Model\ProduitQuery;

/**
 * @method ProduitQuery orderById($order = Criteria::ASC) Order by the id column
 * @method ProduitQuery orderByNom($order = Criteria::ASC) Order by the nom column
 * @method ProduitQuery orderByAutheur($order = Criteria::ASC) Order by the autheur column
 ...
 * @method ProduitQuery groupByNom() Group by the nom column
 * @method ProduitQuery groupByAutheur() Group by the autheur column
 ...
 *
 * @method Produit findOneByNom(string $nom) Return the first Produit filtered by the nom column
 * @method Produit findOneByAutheur(string $autheur) Return the first Produit filtered by the autheur column
 ...
 * @method array findByNom(string $nom) Return Produit objects filtered by the nom column
 * @method array findByAutheur(string $autheur) Return Produit objects filtered by the autheur column
...
abstract class BaseProduitQuery extends ModelCriteria
{
    ...
    public function __construct($dbName = null, $modelName = null, $modelAlias = null)
    {
        ...
        if (null === $modelName) {
            $modelName = 'Acme\\UserBundle\\Model\\Produit';
        }
    ...
    {
        $sql = 'SELECT `id`, `nom`, `autheur`, `description` FROM `produit` WHERE `id` = :p0';
        try {
    ...
    public function filterByNom($nom = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(ProduitPeer::NOM, $nom, $comparison);
    }

    /**
     * Filter the query on the autheur column
     *
     * Example usage:
     * <code>
     * $query->filterByAutheur('fooValue');   // WHERE autheur = 'fooValue'
     * $query->filterByAutheur('%fooValue%'); // WHERE autheur LIKE '%fooValue%'
     * </code>
     *
     * @param     string $autheur The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ProduitQuery The current query, for fluid interface
     */
    public function filterByAutheur($autheur = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($autheur)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $autheur)) {
                $autheur = str_replace('*', '%', $autheur);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(ProduitPeer::AUTHEUR, $autheur, $comparison);
    }
